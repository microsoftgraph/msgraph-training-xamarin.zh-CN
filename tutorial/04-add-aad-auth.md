...
在 "**解决方案资源管理器**" 中，展开 " **GraphTutorial** " 项目并右键单击 "**模型**" 文件夹。 选择 "**添加 > 类 ...**"。命名该类`OAuthSettings`并选择 "**添加**"。 打开 " **OAuthSettings.cs** " 文件，并将其内容替换为以下内容。
...
将新函数添加到**App**类以初始化`GraphServiceClient`。
...
现在，更新`SignIn` **App.xaml.cs**中的函数以调用此函数， `GetUserInfo`而不是。 从`SignIn`函数中删除以下项。
